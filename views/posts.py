...
class PostListEndpoint(Resource):
    ...
    def get(self): 
        if request.args.get('limit'):
            ...
            if not limit.isdigit() or int(limit) > 50 or int(limit) < 0:
...
class PostDetailEndpoint(Resource):
    ...
    def delete(self, id):
        ...
        if not post:
            return Response(json.dumps({"message":"id={0} is invalid".format(id)}), mimetype="application/json", status=404)
        # you should only be able to edit and delete posts that belongs to you
        if post.user_id != self.current_user.id:
            return Response(json.dumps({"message":"id={0} is invalid".format(id)}), mimetype="application/json", status=404)
    ...
    def get(self, id):
        ...
        # post_ids = []
        # for one in Post.query.all():
        #     post_ids.append(one.id)
        # if id > len(Post.query.all()) or id < 0 or id not in post_ids:
        ...
        #these are the users I'm connected to:
        user_ids = get_authorized_user_ids(self.current_user)
